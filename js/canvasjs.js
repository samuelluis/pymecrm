...
(function () {
	...
	function Chart(containerId, options) {
		...
		this._defaultCur		var value = propertyName in object ? object[propertyName] : fallbackObject[propertyName];
				...
				if (xy.x >= bounds.x1 && xy.x <= bounds.x2 && xy.y >= bounds.y1 && xy.y <= bounds.y2) {
					...
					if (type === "mousedown" && eventParam.capture === true) {
						...
						if (eventParam.chart.overlaidCanvas.releaseCapture)
							...
							document.body.removeEventListener("mouseup", this._mouseEventHandler, falseturn;
				...
				if (xy.x >= bounds.x1 && xy.x <= bounds.x2 && xy.y >= bounds.y1 && xy.y <= bounds.y2) {
					...
					if (type === "mousedown" && eventParam.capture === true) {
						...
						if (eventParam.chart.overlaidCanvas.releaseCapture)
							...
							document.body.removeEventListener("mouseup", this._mouseEventHandler, falsath.abs(selectedRegion.x1 - selectedRegion.x2) > 1) {
	...
	Chart.prototype.renderBubble = function (plotUnit) {
		...
		if (isCanvasSupported) {
			this._eventManager.ghostCtx.rect(plotArea.x1, plotArea.y1, plotArea.width, plotArea.height);
			this._eventManager.ghosrea = false;
					}
					else {
	...
	y, Math.abs(plotAreaCoordinates.x1 - plotAreaCoordinates.x2), Math.abs(interlacedGridStartPoint.y - interlacedGridEndPoint.y));
			}
			//throw "123";
		}
	}
